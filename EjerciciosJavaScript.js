...
class Pelicula {
  ...
  validarArreglo(propiedad, valor) {
    ...
    if (valor.length === 0)
      return console.error(`El valor "${valor}" no tiene datos`);
  ...
  validarIMDB(id) {
    if (this.validarCadena("IMDB id", id)) {
      if (!/^([a-z]){2}([0-9]){7}$/.test(id)) {
        return console.error(
          `IMBD id "${id}" no es válido, debe tener 9 carácteres, los 2 primeros letras minúsculas y los 7 restantes números`
        );
      }
    }
  }
